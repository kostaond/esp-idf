...
typedef enum {
    WIFI_START_CTX_HANDLER,
    WIFI_STOP_CTX_HANDLER,
    WIFI_CONNECT_CTX_HANDLER,
    WIFI_DISCONNECT_CTX_HANDLER,
    WIFI_CTX_HANDLERS_END_LIST
}wifi_ctx_handl_type_t;
...
static esp_err_t esp_netif_br_glue_set_instance_handlers_wifi(esp_netif_br_glue_handle_t esp_netif_br_glue)
{
    ...
    esp_err_t ret;
    ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_AP_START, port_action_start, esp_netif_br_glue, &esp_netif_br_glue->wifi_ctx_handlers[WIFI_START_CTX_HANDLER]);
    if (ret != ESP_OK) {
        goto fail;
    }

    ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_AP_STOP, port_action_stop, esp_netif_br_glue, &esp_netif_br_glue->wifi_ctx_handlers[WIFI_STOP_CTX_HANDLER]);
    if (ret != ESP_OK) {
        goto fail;
    }

    ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_AP_STACONNECTED, port_action_connected, esp_netif_br_glue, &esp_netif_br_glue->wifi_ctx_handlers[WIFI_CONNECT_CTX_HANDLER]);
    if (ret != ESP_OK) {
        goto fail;
    }

    ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_AP_STADISCONNECTED, port_action_disconnected, esp_netif_br_glue, &esp_netif_br_glue->wifi_ctx_handlers[WIFI_DISCONNECT_CTX_HANDLER]);
    if (ret != ESP_OK) {
        goto fail;
    }
...
esp_err_t esp_netif_br_glue_add_wifi_port(esp_netif_br_glue_handle_t netif_br_glue, esp_netif_t *esp_netif_port, void *config)
{
    ...
    netif_br_glue->wifi_ctx_handlers = calloc(WIFI_CTX_HANDLERS_END_LIST, sizeof(esp_event_handler_instance_t));
    ESP_GOTO_ON_FALSE(netif_br_glue->wifi_ctx_handlers, ESP_ERR_NO_MEM, fail, TAG, "no memory for WiFi event context handlers");
...
esp_err_t esp_netif_br_glue_del(esp_netif_br_glue_handle_t netif_br_glue)
{
    ...
    if (netif_br_glue->wifi_esp_netif != NULL) {
        ...
        free(netif_br_glue->wifi_ctx_handlers);
    }
