...
typedef enum {
    DM9051_RX_AUTO_BUF_COPY_LEN = 0,
    DM9051_RX_AUTO_BUF_BYTE_CNT
} dm9051_auto_buf_members_t;
...
static esp_err_t emac_dm9051_transmit(esp_eth_mac_t *mac, uint8_t *buf, uint32_t length)
{
    ...
    ESP_GOTO_ON_ERROR(dm9051_memory_write(emac, emac->tx_buffer, length), err, TAG, "write memory failed");
#else
...
static esp_err_t dm9051_alloc_recv_buf(emac_dm9051_t *emac, uint8_t **buf, uint32_t *length)
{
    ...
    *buf = malloc(copy_len);
    if (*buf != NULL) {
        ((uint32_t* )*buf)[DM9051_RX_AUTO_BUF_COPY_LEN] = copy_len;
        ((uint32_t* )*buf)[DM9051_RX_AUTO_BUF_BYTE_CNT] = byte_count;
    } else {
        ret = ESP_ERR_NO_MEM;
        goto err;
    }
...
static esp_err_t emac_dm9051_receive(esp_eth_mac_t *mac, uint8_t *buf, uint32_t *length)
{
    ...
    if (*length != DM9051_ETH_MAC_RX_BUF_SIZE_AUTO) {
        ...
        copy_len = ((uint32_t *)buf)[DM9051_RX_AUTO_BUF_COPY_LEN];
        byte_count = ((uint32_t *)buf)[DM9051_RX_AUTO_BUF_BYTE_CNT];
    }
