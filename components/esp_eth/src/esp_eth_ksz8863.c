...
esp_err_t ksz8863_tail_tag_port_forward(esp_eth_handle_t host_eth_handle, uint8_t *buffer, uint32_t length, void *priv)
{
    ...
    // if port is not found or not registered, drop the packet
    free(buffer);
    return ESP_OK;
}

// Normally, transmit functions are located in emac layer in ESP driver. However, ESP Host Ethernet interface is used to
// perform actual transmit to KSZ, hence the KSZ tx function is located eth layer.
esp_err_t ksz8863_eth_transmit_tag(esp_eth_handle_t host_eth_handle, void *buf, size_t length, uint8_t tail_tag)
{
    ...
    if (length < ETH_HEADER_LEN + ETH_MIN_PAYLOAD_LEN) {
        ...
        // TODO: add mutex here since host_eth_handle is shared resource
        ret = esp_eth_transmit_special(host_eth_handle, 2, buf, length, buff_tail, tail_len);
        free(buff_tail);
    } else {
        // TODO: add mutex here since host_eth_handle is shared resource
        ret = esp_eth_transmit_special(host_eth_handle, 2, buf, length, &tail_tag, 1);
    }
err:
    return ret;
}
