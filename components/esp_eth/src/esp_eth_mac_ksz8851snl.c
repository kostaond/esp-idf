...
typedef enum {
    KSZ8851_RX_AUTO_BUF_COPY_LEN = 0,
    KSZ8851_RX_AUTO_BUF_BYTE_CNT
} ksz8851_auto_buf_members_t;
...
static esp_err_t emac_ksz8851snl_transmit(esp_eth_mac_t *mac, uint8_t *buf, uint32_t length)
{
    ...
    ESP_GOTO_ON_FALSE(length <= KSZ8851_QMU_PACKET_LENGTH, ESP_ERR_INVALID_ARG, err, TAG, "frame is too big (%u)", length);
...
static esp_err_t emac_ksz8851_alloc_recv_buf(emac_ksz8851snl_t *emac, uint8_t **buf, uint32_t *length)
{
    ...
    *buf = malloc(copy_len);
    if (*buf != NULL) {
        ((uint32_t* )*buf)[KSZ8851_RX_AUTO_BUF_COPY_LEN] = copy_len;
        ((uint32_t* )*buf)[KSZ8851_RX_AUTO_BUF_BYTE_CNT] = byte_count;
    } else {
        ret = ESP_ERR_NO_MEM;
        goto err;
    }
...
static esp_err_t emac_ksz8851_receive(esp_eth_mac_t *mac, uint8_t *buf, uint32_t *length)
{
    ...
    if (*length != KSZ8851_ETH_MAC_RX_BUF_SIZE_AUTO) {
        ...
        copy_len = ((uint32_t *)buf)[KSZ8851_RX_AUTO_BUF_COPY_LEN];
        byte_count = ((uint32_t *)buf)[KSZ8851_RX_AUTO_BUF_BYTE_CNT];
    }
