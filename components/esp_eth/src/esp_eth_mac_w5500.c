...
typedef enum {
    W5500_RX_AUTO_BUF_OFFSET = 0,
    W5500_RX_AUTO_BUF_COPY_LEN,
    W5500_RX_AUTO_BUF_RX_LEN,
    W5500_RX_AUTO_BUF_REMAIN
} emac_w5500_auto_buf_members_t;
...
static esp_err_t emac_w5500_transmit(esp_eth_mac_t *mac, uint8_t *buf, uint32_t length)
{
    ...
    ESP_GOTO_ON_FALSE(length <= ETH_MAX_PACKET_SIZE, ESP_ERR_INVALID_ARG, err, TAG, "frame is too big (%u)", length);
...
#if CONFIG_SPIRAM_USE_MALLOC
    memcpy(emac->tx_buffer, buf, length);
    ESP_GOTO_ON_ERROR(w5500_write_buffer(emac, emac->tx_buffer, length, offset), err, TAG, "write frame failed");
#else
...
static esp_err_t emac_w5500_alloc_recv_buf(emac_w5500_t *emac, uint8_t **buf, uint32_t *length)
{
    ...
    *buf = NULL;
    ...
    if (remain_bytes) {
        ...
        *buf = malloc(copy_len);
        if (*buf != NULL) {
            ((uint32_t* )*buf)[W5500_RX_AUTO_BUF_OFFSET] = offset;
            ((uint32_t* )*buf)[W5500_RX_AUTO_BUF_COPY_LEN] = copy_len;
            ((uint32_t* )*buf)[W5500_RX_AUTO_BUF_RX_LEN] = rx_len;
            ((uint32_t* )*buf)[W5500_RX_AUTO_BUF_REMAIN] = remain_bytes;
        } else {
            ret = ESP_ERR_NO_MEM;
            goto err;
        }
...
static esp_err_t emac_w5500_receive(esp_eth_mac_t *mac, uint8_t *buf, uint32_t *length)
{
    ...
    if (*length != W5500_ETH_MAC_RX_BUF_SIZE_AUTO) {
        ...
        offset = ((uint32_t* )buf)[W5500_RX_AUTO_BUF_OFFSET];
        copy_len = ((uint32_t* )buf)[W5500_RX_AUTO_BUF_COPY_LEN];
        rx_len =  ((uint32_t* )buf)[W5500_RX_AUTO_BUF_RX_LEN];
        remain_bytes = ((uint32_t* )buf)[W5500_RX_AUTO_BUF_REMAIN];
    }
    // 2 bytes of header
    offset += 2;
    // read the payload
...
#if CONFIG_SPIRAM_USE_MALLOC
    emac->tx_buffer = heap_caps_malloc(ETH_MAX_PACKET_SIZE, MALLOC_CAP_DMA);
